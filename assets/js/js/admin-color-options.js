/******/ (() => { // webpackBootstrap
/*!***************************************!*\
  !*** ./src/js/admin-color-options.js ***!
  \***************************************/
(function ($) {
  function updateColorIndicator(scheme) {
    const $indicator = $('#color-indicator');
    let selectedPalette = $('#custom-admin-color-picker .color-option input[type=radio]:checked').siblings('.color-palette').html();
  ...
  $(document).ready(function () {
    ...
    $('#custom-admin-color-picker input[type=radio][value=' + initialScheme + ']').prop('checked', true).closest('.color-option').addClass('selected').siblings().removeClass('selected');
  });
})(jQuery);
/******/ })()
;
